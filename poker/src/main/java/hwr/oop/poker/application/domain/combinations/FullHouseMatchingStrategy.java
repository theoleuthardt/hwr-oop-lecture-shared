...
final class FullHouseMatchingStrategy implements CombinationDetectionStrategy {
  ...
  public FullHouseMatchingStrategy(CombinationDetectionStrategy pair,
      CombinationDetectionStrategy trips) {
    this.pair = pair;
    this.trips = trips;
  }
  ...
  private boolean isHandFullHouseBasedOnResults(CombinationDetectionResult pairResult,
      CombinationDetectionResult tripResult) {
  ...
  private boolean areBothSuccessful(CombinationDetectionResult first,
      CombinationDetectionResult second) {
    return first.successful() && second.successful();
  }

  private List<List<Card>> buildAllCandidates(List<List<Card>> allTrips,
      List<List<Card>> allPairs) {
  ...
  private Stream<List<Card>> combineSetsWithPairs(List<List<Card>> allTrips,
      List<List<Card>> allPairs) {
    return allTrips.stream()
